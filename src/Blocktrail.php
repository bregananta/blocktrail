...
class Blocktrail
{
    ...
    public function __construct()
    {
        $this->client = new BlocktrailSDK(
            config('blocktrail.apiPublicKey'),
            config('blocktrail.apiPrivateKey'),
            config('blocktrail.network'),
            config('blocktrail.testnet'),
            config('blocktrail.version'),
            $apiEndpoint = null
        );
    }
     ...
     * @throws \Exception
     */
    public function createWallet($identifier = null, $passphrase)
    {
     ...
     * @throws \Exception
     */
    public function pay(Wallet $wallet, array $outputs, $allowZeroConf = false, $randomizeChangeIdx = true)
    {
        return $wallet->pay($outputs, $allowZeroConf, $randomizeChangeIdx);
    }
     ...
     * @return bool
     */
    public function unlock(Wallet $wallet, $passphrase)
    {
    ...
    public function easyDeleteWallet($identifier, $passphrase)
    {
        $wallet = $this->initWallet($identifier, $passphrase);
